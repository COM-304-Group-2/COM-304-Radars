...
def get_ant_static_2d(num_frames, num_tx, num_rx, adc_samples):
    ...
    z_ant_pos = np.reshape(z_ant_pos, (-1,1))
...
def load_raw_data(data_path):
    ...
    #raw_data = np.max(raw_data, axis=1, keepdims=True)  # Average over frames
...
def load_raw_data_with_frames(data_path):
    import scipy.io as sio
    mat_data = sio.loadmat(data_path)
    ...
    num_frames, num_tx, num_rx, adc_samples = raw_data.shape
    num_x_stp = num_tx * num_rx
    num_z_stp = num_frames  # Because radar is static

    # Reshape to (num_x_stp, num_z_stp, adc_samples) for each frame
    raw_data = raw_data.transpose(1, 2, 0, 3)  # (tx, rx, frames, samples)
    raw_data = raw_data.reshape(num_tx * num_rx, num_frames, adc_samples)  # (12, num_frames, 512)
    ...
    return radar_params, raw_data  # raw_data is now (12, num_frames, 512)
